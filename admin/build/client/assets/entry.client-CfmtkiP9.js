import { r, a as v, j as R } from "./index-DFWxKLZT.js";
import {
	E as h,
	g as C,
	i as b,
 ...
 */ function z(d) {
	...
	for (let [a, e] of w)
		if (e && e.__type === "RouteErrorResponse")
			s[a] = new h(e.status, e.statusText, e.data, e.internal === !0);
	...
	if (!t) {
		if (window.__remixContext.future.unstable_singleFetch) {
			if (!n) {
				...
				b(u, "No stream found for single fetch decoding"),
		...
		if (!window.__remixContext.isSpaMode) {
			...
			let u = E(i, window.location, window.__remixContext.basename);
			if (u)
				for (let l of u) {
					let _ = l.route.id,
					...
					x &&
					F(m, x, window.__remixContext.isSpaMode) &&
		...
		(t = $({
			...
			future: {
				...
				v7_relativeSplatPath:
					window.__remixContext.future.v3_relativeSplatPath,
				v7_skipActionErrorRevalidation:
					window.__remixContext.future.unstable_singleFetch === !0,
			},
			hydrationData: o,
			mapRouteProperties: k,
			unstable_dataStrategy: window.__remixContext.future
				.unstable_singleFetch
				? P(window.__remixManifest, window.__remixRouteModules, () => t)
				: void 0,
			unstable_patchRoutesOnNavigation: H(
